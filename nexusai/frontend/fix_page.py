...
end_idx = content.find('export default function BillingPage')

array_end = content.rfind(']', start_idx, end_idx) + 1

content = content[:start_idx] + new_plans + content[array_end:]

content = content.replace('workspace?.plan === "free" || !workspace?.plan ? "weekly"', 'workspace?.plan === "free" || !workspace?.plan ? "basic"')
content = content.replace('p.id === "weekly"', 'p.id === "basic"')
